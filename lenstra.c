...
void chk_perferctpow(mpz_t *mpz_N);
void fec(char *N_file, char *fb_file, char *res_file);

int main(int argc, char *argv[])
{
  printf("Elliptic Curve Method Implementation v%s for large \
  integers factorization.\n", VERSION);
  printf("Code by Ardoino Paolo <ardoino.gnu@disi.unige.it>\n");
  if (argc < 4)
    help(*argv);
  else if (argc == 4 && strcmp(*(argv + 1), "-g") == 0 && *(argv + 2) \
  && *(argv + 3))
    genprimes(*(argv + 2), *(argv + 3));
  else if (argc == 5 && strcmp(*(argv + 1), "-f") == 0 && *(argv + 2) \
  && *(argv + 3) && *(argv + 4))
...
void fec(char *N_file, char *fb_file, char *res_file)
{
  ...
  printf("Non primo bastardo offri!\n");
  while (1) {
    /*if (foundfactor == 1) {
	...
	if (mpz_cmp_ui(mpz_N, 1) == 0) {
          gettimeofday(&tm1, NULL);
          //printf("Factorization has been completed in %ld \
	  microseconds.\n",	  (tm1.tv_sec*1000000 + tm1.tv_usec)\
	  - (tm0.tv_sec*1000000 + tm0.tv_usec));
          //fprintf(fp, "Factorization has been completed in %ld \
	  microseconds.\n", (tm1.tv_sec*1000000 + tm1.tv_usec) - \
	...
	while (pctr < pnum) {
	  if (*(prime + pctr) <= 1) {
	    ...
	    mpz_set_ui(exp, mpz_sizeinbase(mpz_N, 3));
	    mpz_div_ui(exp, exp, (long int)logl((long double) \
	    *(prime + pctr++)));
            for (ctr = 1; mpz_cmp_ui(exp, ctr) > 0; ctr++) {
              mpz_mul(x, x1, x1);
	      mpz_mul_ui(x, x, 3);
              mpz_add(x, x, a);
	      mpz_mul_ui(y, y1, 2);
		  ...
		  microseconds.\n", (tm1.tv_sec*1000000 + tm1.tv_usec)\
		  - (tm0.tv_sec*1000000 + tm0.tv_usec)); 
                  //fprintf(fp, "Factorization has been completed in \
		  %ld microseconds.\n", (tm1.tv_sec*1000000 + \
		  tm1.tv_usec) - (tm0.tv_sec*1000000 + tm0.tv_usec)); 
                  printf("Factorization has been completed in\
		  %ld seconds.\n", tm1.tv_sec - tm0.tv_sec); 
                  fprintf(fp, "Factorization has been completed\
		  in %ld seconds.\n", tm1.tv_sec - tm0.tv_sec); 
                  mpz_clear(mpz_N);
                  mpz_clear(a);
                  mpz_clear(b);
